...
const Toolbar: React.FC<ToolbarProps> = ({ onAddNode, disabled = false }) => {
  ...
  const tools = [
    { type: ElementType.Screen, label: 'Screen', icon: <ScreenIcon /> },
    { type: ElementType.Command, label: 'Command', icon: <CommandIcon /> },
    { type: ElementType.EventInternal, label: 'Internal Event', icon: <EventInternalIcon /> },
    { type: ElementType.ReadModel, label: 'Read Model', icon: <ReadModelIcon /> },
    { type: ElementType.EventExternal, label: 'External Event', icon: <EventExternalIcon /> },
  ];
        ...
        <div className="flex flex-col items-center gap-3 bg-white/80 backdrop-blur-sm shadow-lg rounded-full p-2">
          {tools.map((tool, index) => (
            <div key={tool.type} className="flex items-center gap-3 w-full justify-end">
              <span className="hidden md:inline text-sm font-medium bg-gray-700 text-white py-1 px-3 rounded-md shadow-sm transition-opacity duration-300">
                {tool.label}
              </span>
              <button
