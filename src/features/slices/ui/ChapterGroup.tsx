...
interface ChapterGroupProps {
    chapterName: string;
    slices: Slice[];
    sliceBounds: Map<string, { x: number, y: number, width: number, height: number, minX: number, maxX: number, minY: number, maxY: number }>;
...
const ChapterGroup: React.FC<ChapterGroupProps> = ({
    ...
    // 1. Calculate Chapter Bounds union of all slice VISUAL bounds
    // We wrap the already-padded slice containers.
    ...
    // Nesting Buffer: Distance between Slice boundary and Chapter boundary
