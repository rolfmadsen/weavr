...
export function useGunState(modelId: string | null) {
    ...
    const updateNode = useCallback((nodeId: string, updates: Partial<Node>) => {
        ...
        gunService.getModel(modelId).get('nodes').get(nodeId).put(sanitizedUpdates);
    }, [modelId]);
    ...
    const addLink = useCallback((sourceId: string, targetId: string, label: string, id?: string) => {
        ...
        gunService.getModel(modelId).get('links').get(linkId).put(newLinkData);
        return linkId;
    }, [modelId]);
    ...
    const updateLink = useCallback((linkId: string, updates: Partial<Link>) => {
        ...
        gunService.getModel(modelId).get('links').get(linkId).put(sanitizedUpdates);
    }, [modelId]);
    ...
    const updateNodePosition = useCallback((nodeId: string, x: number, y: number) => {
        ...
        gunService.getModel(modelId).get('nodes').get(nodeId).put({ x: safeX, y: safeY, fx: safeX, fy: safeY });
    }, [modelId]);
    ...
    const addSlice = useCallback((title: string, order: number) => {
        ...
        gunService.getModel(modelId).get('slices').get(sliceId).put(newSliceData);
        return sliceId;
    }, [modelId]);
    ...
    const updateSlice = useCallback((sliceId: string, updates: Partial<Slice>) => {
        ...
        gunService.getModel(modelId).get('slices').get(sliceId).put(sanitizedUpdates);
    }, [modelId]);
    ...
    const deleteSlice = useCallback((sliceId: string) => {
        ...
        gunService.getModel(modelId).get('slices').get(sliceId).put(null as any);
    ...
    const addDefinition = useCallback((def: Omit<DataDefinition, 'id'>) => {
        ...
        gunService.getModel(modelId).get('definitions').get(defId).put(newDefData);
        return defId;
    }, [modelId]);
    ...
    const deleteDefinition = useCallback((defId: string) => {
        ...
        gunService.getModel(modelId).get('definitions').get(defId).put(null as any);
    }, [modelId]);
